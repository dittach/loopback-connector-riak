...
Riak.prototype.all = function(modelName, conditions, apiCallback){
  ...
  async.waterfall([
    search,
    mapResultsToFindQueries,
    parallelize
  ], apiCallback);

  function search(callback){
    self._search(modelName, conditions.where, function(error, results){
      if (error) return callback(error);

      // TODO: extract results IDs
      callback(null, results);
    });
  }

  function mapResultsToFindQueries(searchResultIds, callback){
    callback(null, searchResultIds.map(function(id){
      return function(getCallback){
        self.find(modelName, id, getCallback);
      }
    }));
  }

  function parallelize(findQueries, callback){
    async.parallel(findQueries, callback);
  }
};
...
Riak.prototype.destroyAll = function(model, where, apiCallback){
  ...
  this._notImplemented("destroyAll");
};
