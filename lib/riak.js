...
Riak.prototype._onConnectionSuccess = function(error){
  this.log('Riak connection failed:', error);
};

Riak.prototype._onConnectionFailure = function(){
  this.log('Riak connection is established:', this.settings.host.toString(), this.settings.port.toString());
};

Riak.prototype._onUnexpectedError = function(activity){
  this.log('Unexpected error while', activity);
};
...
Riak.prototype.create = function(modelName, data, apiCallback){
  ...
  function checkExistance(callback){
    self.db.exists(modelName, key, function(error, exists){
      if (exists) return callback(self.log("Key conflict during create for key", key));
...
Riak.prototype.updateAttributes = function(modelName, data, apiCallback){
  ...
  function find(callback){
    self.db.find(modelName, key, function(error, result){
      if (error)   return callback(error);
      if (!result) return callback(self.log("updateAttributes failed to find a Riak document for key", key));
