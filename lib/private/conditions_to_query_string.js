...
module.exports = function(riak, bucketName, query){
  ...
  var notQueries = query.not || null;

  // delete the not so that it will not interfere with componentization
  delete query.not;

  var queries = query.and || query.where || query;
  ...
  function componentizeFacets(query){
    ...
    if (notQueries) {
      var notStatement = " NOT (" + componentizeNotQuery(notQueries).join(" AND ") + " )";
      
      facetComponents.push(notStatement);
    }
